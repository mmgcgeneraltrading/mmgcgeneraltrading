...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const active=(window.MMGC_JOBS||[]).filter(x=>x.deadline&&new Date(x.deadline)>=now).sort((a,b)=>new Date(a.deadline)-new Date(b.deadline));
  let type='all';
  ...
  function render(){
    ...
    const rows=active.filter(x=>(type==='all'||x.type===type)&&(!q||[x.title,x.employer,x.location,x.summary,x.arrangement,x.primarySourceName,x.secondarySourceName].join(' ').toLowerCase().includes(q)));
    ...
    list.innerHTML=rows.map(x=>{
      ...
      return `<article class="job-card ${left<=2?'urgent-job':''}" id="${esc(x.type)}-${esc(x.id)}"><div><div class="job-card-flags"><span class="label">${x.type==='consultancy'?'Consultancy':'Job vacancy'}</span><span class="verification-badge ${verifyClass(x)}">✓ ${verifyText(x)}</span></div><h3><a href="${detail}">${esc(x.title)}</a></h3><div class="job-meta"><span><b>${esc(x.employer)}</b></span><span>${esc(x.location)}</span><span>${esc(x.arrangement)}</span></div><p>${esc(x.summary)}</p><div class="verification-row"><b>Checked:</b> ${esc(sources||'Official/reputable source')} · 10 Sep 2026</div><div class="job-meta"><span class="deadline">${closing} · ${esc(x.deadlineLabel||dl.toLocaleString('en-GB',{dateStyle:'medium',timeStyle:'short'}))}</span></div></div><div class="job-actions"><a class="primary" href="${detail}">View Details</a><a href="https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(x.title)}&dates=${dl.toISOString().replace(/[-:]/g,'').replace('.000','')}/${dl.toISOString().replace(/[-:]/g,'').replace('.000','')}&details=${encodeURIComponent('Deadline for '+x.title+' - verify on official source: '+x.official)}" target="_blank" rel="noopener">Calendar</a><a href="https://wa.me/?text=${encodeURIComponent(x.title+' — '+x.employer+' — deadline '+(x.deadlineLabel||dl.toLocaleString('en-GB'))+' — '+location.origin+'/'+detail)}" target="_blank" rel="noopener">Share</a><a href="${esc(x.official)}" target="_blank" rel="noopener">${esc(x.officialLabel||'Official Source')}</a>${second}</div></article>`;
