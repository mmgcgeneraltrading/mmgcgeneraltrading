document.addEventListener('DOMContentLoaded', () => {
  ...
  if (nav && !nav.querySelector('a[href="past-tenders.html"]')) {
    const pastLink = document.createElement('a');
    pastLink.href = 'past-tenders.html';
    pastLink.textContent = 'Past Tenders';
    const quoteLink = nav.querySelector('.pill');
    nav.insertBefore(pastLink, quoteLink || null);
  }
  ...
  cards.forEach((card) => {
    ...
    if (closeRaw) {
      ...
      if (msLeft <= twoDaysMs) {
        ...
        activeAlerts.push({kind:'SUBMISSION DUE SOON',title,detail:hours<=24?`Submission closes in about ${hours} hour${hours===1?'':'s'}.`:'Submission closes within 2 days.',href:`tender-details.html?id=${encodeURIComponent(card.dataset.tenderId||'')}`});
      }
    }
    if (siteRaw) {
      ...
      if (msToVisit >= 0 && msToVisit <= twoDaysMs) {
        ...
        activeAlerts.push({kind:'SITE VISIT DUE SOON',title,detail:hours<=24?`Site visit is in about ${hours} hour${hours===1?'':'s'}.`:'Compulsory site visit is within 2 days.',href:`tender-details.html?id=${encodeURIComponent(card.dataset.tenderId||'')}`});
      }
    }
  });

  if (alerts && activeAlerts.length) {
    alerts.innerHTML = `<div class="wrap alerts-wrap">${activeAlerts.map(a=>`<a class="deadline-alert" href="${a.href}"><span class="alert-badge">${a.kind}</span><strong>${a.title}</strong><span>${a.detail}</span><b>View summary →</b></a>`).join('')}</div>`;
  }
